...
<Note>
Deploying a list does not make it visible on the Curate frontend. The public list-of-lists mechanism uses Curate Classic (`GeneralizedTCR`), not Light Curate's `addItem(string)`. Note that creating new lists through Kleros frontends is being disabled as part of the Curate V1 deprecation (see the notice at the top of this page).
</Note>
