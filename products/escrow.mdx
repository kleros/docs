...
<Note>
The Kleros Escrow applications (V1 and V2) are not actively maintained by the Kleros team. The smart contracts remain deployed and the source code is open, so any contributor can review the code and build their own interface on top of it. [Escrowly](/Community%20&%20Ecosystem/escrowly) is one example of a community-built Escrow frontend.
</Note>
