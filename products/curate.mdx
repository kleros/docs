...
- **Stake Curate** (V1 infrastructure) - continuous verification with permanent deposits, on Gnosis Chain. Deployed mid-August 2025; first implementation with DAMM Capital (September 2025). See [Stake Curate](/products/stake-curate).
...
<Tabs>
  ...
  <Tab title="Contract-Domain Names">
    **Problem:** Users and dApps can't easily tell which website a contract officially belongs to.
    
    **Solution:** The Contract-Domain Name (CDN) registry verifies contract-to-domain pairings, along with project metadata such as logos and descriptions.
    
    **Example:** Verified project information and logos served to wallets and dApps across the ecosystem.
  </Tab>
</Tabs>
...
| **Stake Curate** | V1 infra | Continuous verification with permanent deposits | - | See [Stake Curate](/products/stake-curate) |
...
**Curate V1 deprecation:** the Classic Curate and Light Curate contracts remain deployed and existing lists (including the Scout registries) keep running. Kleros will not launch new registries on Curate V1, and list creation is being removed from Kleros frontends, with banners in the Curate V1 app flagging the change.

Following **KIP-87**, a dedicated **Curation Court with hidden voting** went live on Gnosis Chain in mid-2026. Hidden voting splits juror voting into commit-and-reveal phases so jurors can't see other votes before deciding, reducing herding on curation disputes. The Scout registries migrated to this court on **July 13, 2026**. Curate V2 development also aligned its evidence query with the kleros-core schema migration.
...
<CardGroup cols={2}>
  <Card title="Launch Curate (V1)" icon="arrow-up-right-from-square" href="https://curate.kleros.io/">
    Browse, submit to, and challenge items on existing V1 lists
  </Card>
  <Card title="Curate V2" icon="flask" href="https://curate-v2.netlify.app/">
    Try the next-generation Curate interface
  </Card>
